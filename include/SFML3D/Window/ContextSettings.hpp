...
/// settings have no impact on the regular SFML3D rendering
/// (graphics module) -- except the anti-aliasing level, so
/// you may need to use this structure only if you're using
/// SFML3D as a windowing system for custom OpenGL rendering.
///
...
/// are not supported by the system; instead, SFML3D will try to
