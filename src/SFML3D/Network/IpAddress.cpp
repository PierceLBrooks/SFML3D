...
#include <SFML3D/Network/SocketImpl.hpp>
#include <cstring>


namespace
{
    sf3d::Uint32 resolve(const std::string& address)
    {
        ...
        else
        {
            ...
            sf3d::Uint32 ip = inet_addr(address.c_str());
            if (ip != INADDR_NONE)
                return ip;
...
m_address(0)
{
    ...
    // SFML3D doesn't publicly use INADDR_ANY (it is always used implicitely)
}
...
IpAddress IpAddress::getPublicAddress(Time timeout)
{
    ...
    // Here we get the web page from http://www.sfml3d-dev.org/ip-provider.php
    // and parse the result to extract our IP address
    // (not very hard: the web page contains only our IP address).

    Http server("www.sfml3d-dev.org");
