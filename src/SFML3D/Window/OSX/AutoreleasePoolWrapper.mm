...
/// We use an sf3d::ThreadLocalPtr to have one PoolWrapper in each thread.
///
...
#ifdef SFML3D_DEBUG
    if (m_count < 0) {
        sf3d::err() << "~PoolWrapper : m_count is less than zero! "
                     "You called releasePool from a thread too many times."
                  << std::endl;
    } else if (m_count > 0) {
        sf3d::err() << "~PoolWrapper : m_count is greater than zero! "
                     "You called releasePool from a thread to few times."
                  << std::endl;
    } else { // m_count == 0
        sf3d::err() << "~PoolWrapper is HAPPY!" << std::endl;
    }
#endif
}
...
#ifdef SFML3D_DEBUG
    if (m_count <= 0) {
        sf3d::err() << "PoolWrapper::retain : m_count <= 0! " << std::endl;
    }
#endif
}
...
#ifdef SFML3D_DEBUG
    if (m_count < 0) {
        sf3d::err() << "PoolWrapper::release : m_count < 0! " << std::endl;
    }
#endif
}
...
namespace
{
    ...
    sf3d::ThreadLocalPtr<sf3d::priv::PoolWrapper> localPool;
}
...
void retainPool(void)
{
    ...
    if (localPool == NULL) {
        localPool = new sf3d::priv::PoolWrapper();
    }
...
#ifdef SFML3D_DEBUG
    if (localPool == NULL) {
        sf3d::err() << "releasePool : You must call retainPool at least once "
...
void drainPool()
{
    if (localPool != NULL) {
        ...
        sf3d::err() << "releasePool must be called at least one before drainPool"
                  << std::endl;
    }
#endif
}
